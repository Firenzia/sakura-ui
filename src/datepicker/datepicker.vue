        ...
        <div class="date-panel-body">
          <div v-if="model === 'day'">
            ...
            <div class="date-panel-date" v-for="(item1, i) in helper.getRange(1,6)" :key="i">
              ...
              >{{getVisibleDate (i, j).getDate()}}</span>
            </div>
          </div>
          <!-- year -->
...
export default {
  ...
  data() {
    return {
      ...
      display: {
        year:
          (this.value && this.value.getFullYear()) || new Date().getFullYear(),
  ...
  computed: {
    formattedValue: {
      ...
      get: function() {
        return this.value instanceof Date
          ? helper.getFormatDate(this.value)
          : "";
      },
      set: function(newVal) {}
    },
    visibleDays() {
      ...
      var curMonthFirstDayDay =
        helper.getDay(curMonthFirstDay) === 0
          ? 7
          : helper.getDay(curMonthFirstDay);
      let x = curMonthFirstDayDay - 1; // 前面需要补多少位
      var arr = [];
      for (let i = 0; i < 42; i++) {
        arr.push(
          new Date(curMonthFirstDay.getTime() + (-x + i) * 3600 * 24 * 1000)
        );
      }
      return arr;
    },
  ...
  methods: {
    ...
    onClickFirstYear() {
      ...
      this.yearsList = helper.getRange(
        currentFirstYear - 10,
        currentFirstYear + 1
      );
    },
    ...
    setValueManually($event) {
      if (!helper.isValidDate($event)) {
        ...
        this.$refs.inputWrapper.$refs.input.value = this.isDate(this.value)
          ? helper.getFormatDate(this.value)
          : "";
        return;
      }
    ...
    selectYear(newY) {
      // 年面板不能选第一项和最后一项
      if (
        newY === this.yearsList[0] ||
        newY === this.yearsList[this.yearsList.length - 1]
      ) {
        return;
      }
